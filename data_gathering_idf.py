...
def get_word_dict_proper(filename, proper_words, idf):
    '''
    Counts the occurances of a word in a file given filename
    Arguments:
        filename (str): The name or path of a file you want to process
    ...
    Returns:
        arr: The occurence array for that file
    Examples:
    ...
    for i, count in enumerate(return_dict.values()):
        return_arr.append((count / total_words) * idf[i])  # tf * idf
    return return_arr
...
def process_data_folder(folder_name, class_dict, popular_words):
    ...
    Arguments:
        ...
        popular_words (pd.Dataframe): popular_words df
    Returns:
    ...
    proper_words = popular_words.values[:, 0]
    idf = popular_words.values[:, 3]
    return_cols = np.array(['document_id', 'prediction_class'] +
                           list(proper_words), dtype=np.dtype('str'))
    return_vals = []
        ...
        for data_class in tqdm(
            ...
            with cd(data_class):
                for filename in [
                    ...
                    temp_word_arr = get_word_dict_proper(
                        filename, proper_words, idf)

                    # Creating an array of values
                    return_vals.append(
                        np.array([filename, class_dict[data_class]] +
                                 temp_word_arr, dtype=np.dtype('float32')))
...
plt.show(temp_df.plot.scatter(x='count', y='count_files'))
# %%Resulting dataframe
logging.info('Doing idf')
idf_ = []
for row in tqdm(word_df.iterrows(), desc='Computing idf', unit='row', file=sys.stdout, total=word_df.shape[0]):
    idf_.append(math.log(word_df.shape[0] / row[1]['count_files']))
word_df['idf'] = idf_
word_df
logging.info(f'Selecting {NUMBER_TOP_WORDS} most popular words:')
uwords_df = top_rows(word_df, NUMBER_TOP_WORDS, True)
...
with cd('data'):
    ...
    train_cols, train_vals = process_data_folder('train', class_dict, uwords_df)
    ...
    test_cols, test_vals = process_data_folder('test', class_dict, uwords_df)
...
options = {
    ...
    'train_fname': 'tfidf_train_df',
    'test_fname': 'tfidf_test_df',
